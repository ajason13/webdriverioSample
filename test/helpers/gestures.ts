 ...
 */
export async function checkIfDisplayedWithSwipe ({
  scrollContainer,
  searchableElement,
  maxScrolls,
  amount = 0,
  ...
  if (!(await searchableElement.isDisplayed()) && amount <= maxScrolls) {
    ...
    throw new Error('The element could not be found or is not visible.')
    // throw new Error(`The element '${searchableElement.selector}' could not be found or is not visible.`);
  }
